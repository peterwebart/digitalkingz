 ...
 * /people/search with the values in the query string. That makes every filtered
...
export function PeopleSearchForm({
  ...
  return (
    <form action="/people/search" method="GET" className="surface-card flex flex-col gap-4 p-6">
      <div className="flex flex-col gap-3 sm:flex-row">
        ...
        <input
          ...
          placeholder="Search people, usernames, industries…"
...
export function AlphabetBar({ active }: { active?: string }) {
  return (
    <nav aria-label="Browse people by first letter" className="flex flex-wrap gap-1.5">
      {ALPHABET.map((letter) => {
        const selected = active?.toUpperCase() === letter
        return (
          <Link
            key={letter}
            href={`/people/search?letter=${letter}`}
