...
export const metadata: Metadata = {
  ...buildMetadata({
    title: 'Search the influencer directory',
...
export default async function PeopleSearchPage({
  ...
  const urlWith = (changes: Record<string, string | number | undefined>) => {
    ...
    return `/influencers/search${qs ? `?${qs}` : ''}`
  }

  return (
    <>
      <PageHero
        eyebrow="Influencer directory"
        ...
        breadcrumbs={[
          ...
          { name: 'Search', path: '/influencers/search' },
        ]}
      />
        ...
        <div className="flex flex-col gap-8">
          ...
          {result.docs.length === 0 ? (
            <div className="surface-card flex flex-col items-start gap-3 p-8">
              ...
              <Link href="/influencers/search" className="text-sm text-brand-400 hover:text-brand-300">
                Clear all filters
              </Link>
            </div>
          ) : (
