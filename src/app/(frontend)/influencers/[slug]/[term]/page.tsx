...
export async function generateMetadata({
  ...
  return buildMetadata({
    ...
    description:
      term.description ??
      `Browse ${count} ${count === 1 ? 'profile' : 'profiles'} in the Digital Kingz influencer directory for the ${term.title} ${noun}.`,
    path: `/influencers/${segment}/${slug}`,
...
export default async function TermPage({
  ...
  const path = `/influencers/${segment}/${slug}`
  const crumbs = [
    { name: 'Home', path: '/' },
